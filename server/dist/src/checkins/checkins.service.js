...
let CheckInsService = class CheckInsService {
    ...
    async record(relationshipId, userId, mood) {
        ...
        if (partnerMembership) {
            const res = await this.prisma.checkInResponse.findFirst({ where: { relationshipId, authorId: partnerMembership.userId, mood } });
    ...
    async today(relationshipId, userId) {
        const date = new Date();
        date.setHours(0, 0, 0, 0);
        return this.prisma.checkIn.findUnique({ where: { relationshipId_userId_date: { relationshipId, userId, date } } });
    }
    async setResponse(relationshipId, authorId, mood, content) {
        const existing = await this.prisma.checkInResponse.findFirst({ where: { relationshipId, authorId, mood } });
        if (existing) {
            return this.prisma.checkInResponse.update({ where: { id: existing.id }, data: { content } });
        }
        return this.prisma.checkInResponse.create({ data: { relationshipId, authorId, mood, content } });
    }
};
